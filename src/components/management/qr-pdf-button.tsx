...
export function QrPdfButton() {
  ...
  async function getPDF(
    compress: boolean,
    errorSet: (msg: string) => void,
    progressSet: (msg: string) => void,
  ) {
    if (!contestId) {
      errorSet("Select a contest first");
      return;
    }
    if (!apiKey) {
      errorSet("Not authenticated");
      return;
    }
    errorSet("");
    progressSet("Generating… (0/?)");
    ...
    try {
      const [contestRes, participantsRes] = await Promise.all([
        ...
        fetch(`${BASE}/contest/${encodeURIComponent(contestId)}/participants`, {
          headers,
        }),
      ]);
      if (!contestRes.ok)
        throw new Error(`Contest fetch failed: ${contestRes.statusText}`);
      if (!participantsRes.ok)
        throw new Error(
          `Participants fetch failed: ${participantsRes.statusText}`,
        );
      ...
      const trackableParticipants = allParticipants.filter(
        (p) => !p.nonTrackable,
      );
      ...
      if (participantIds.length === 0) {
        errorSet("No registered participants found");
        progressSet("");
        return;
      }
      progressSet(`Loading fonts…`);
      ...
      progressSet(`Generating… (0/${participantIds.length})`);
      ...
      for (let i = 0; i < participantIds.length; i++) {
        ...
        // Only generate first page for preview when compressing
        ...
        try {
          ...
          throw new Error(
            `Failed to render participant ${n}: ${renderError instanceof Error ? renderError.message : "Unknown error"}`,
          );
        }

        progressSet(`Generating… (${i + 1}/${participantIds.length})`);
      }
      if (!compress) {
        doc!.save(`qr-codes-${contestId}.pdf`);
      } else {
        progressSet("Building ZIP…");
      ...
      errorSet(e instanceof Error ? e.message : "Failed");
    } finally {
      progressSet("");
    }
  }
  ...
  async function generate() {
    return getPDF(false, setError, setProgress);
  }

  async function generateZip() {
    return getPDF(true, setZipError, setZipProgress);
  }

  return (
    <Card>
      <CardHeader>
        <h2 className="text-sm font-bold">QR Code Downloads</h2>
      </CardHeader>
      <CardContent className="flex flex-col gap-3 pb-4">
        <p className="text-xs text-zinc-400">
          A4 · 1 card/page · registered participants only
        </p>
        <Button
          variant="primary"
          isDisabled={!!progress}
